...
export class PokemonService {
  ...
  async create(
    ...
    //? createDolarDiarco: CreateDolarDiarcoDto
  ) {
    try {
    ...
    //? const dolarDiarco = await this.dolarDiarcoModel.create(createDolarDiarco)
    //? return dolarDiarco;
  }
  ...
  async remove(term: string) {
    ...
    if (result.deletedCount === 0) throw new BadRequestException(`Pokemon with id ${term} not found`)
    return;
  }
